...
def ssref_algorithm(db, google_id, log_data, all_questions, solved_question_ids, num_recommendations, rec_type):
    ...
    Returns:
        추천된 문제 목록
    """
    if rec_type == 1: # 학습 기록 30개 미만일 때 랜덤 추천
        available_questions = [q for q in all_questions if q.question_id not in solved_question_ids]
        if len(available_questions) <= num_recommendations:
            return available_questions # 추천할 문제 수가 적으면 모두 반환
        else:
            return random.sample(available_questions, num_recommendations)

    # 1. 사용자 정보 가져오기
    user = db.query(models.UserDB).filter(models.UserDB.google_id == google_id).first()
    
    # study_level을 정수로 변환하는 맵
    study_level_map = {'S': 5, 'A': 4, 'B': 3} # 필요에 따라 레벨과 숫자 매핑 조정
    study_level = study_level_map.get(user.study_level, 3) if user and user.study_level else 3
    ...
    # 5. 상위 N개 문제 선택
    recommended_questions = select_top_n_questions(question_scores, num_recommendations, all_questions)
...
def calculate_question_scores(all_questions, solved_question_ids, question_difficulty, question_topic, study_level, topic_success_rate, difficulty_success_rate, rec_type):
    ...
    for question in all_questions:
        ...
        # 명시적으로 정수 타입으로 변환하여 안전성 확보
        try:
            difficulty_int = int(difficulty)
            study_level_int = int(study_level)
        except ValueError:
            # 변환 실패 시 기본값 설정 또는 해당 문제 건너뛰기
            # 여기서는 문제를 건너뛰어 다른 문제에 대한 추천을 시도합니다.
            continue 

        # 난이도 적합성 점수 (학습 수준과 난이도의 차이가 작을수록 높은 점수)
        difficulty_score = 1.0 - abs(difficulty_int - study_level_int) / 5.0
...
def select_top_n_questions(question_scores, n, all_questions):
    """상위 N개 문제 선택"""
    # 점수 기준으로 정렬
